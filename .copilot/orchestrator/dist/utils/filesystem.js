...
import { SessionSchema, ConfigSchema } from '../types.js';
...
// Topic Management
// ============================================
export function getTopicPath(topicName) {
...
export async function loadTopic(topicName) {
...
export async function createTopic(topicName, description) {
    const topicPath = getTopicPath(topicName);
    if (await exists(topicPath)) {
...
export async function saveSession(topicName, session) {
...
// ============================================
async function loadDelegations(delegationsDir) {
...
function parseDelegationFile(filename, content) {
    ...
    return {
        id: `${agent}-${timestamp}`,
        agent,
        task: metadata.task || 'Unknown task',
        context: metadata.context || {},
        model_tier: metadata.model_tier || 'cheap',
        status: metadata.status || 'PENDING',
        created: new Date(parseInt(timestamp)).toISOString(),
    };
}
export async function createDelegation(topicName, agent, task, context, modelTier) {

import { listTopics, loadTopic, createTopic as createTopicFs, saveSession, createDelegation, loadAgent, getAgentPath, } from '../utils/filesystem.js';
import { CreateTopicArgsSchema, SwitchTopicArgsSchema, DelegateToArgsSchema, CheckDelegationsArgsSchema, } from '../types.js';
...
// list_topics
// ============================================
export async function listTopicsTool() {
    ...
    const topicDetails = await Promise.all(topics.map(async (name) => {
        const state = await loadTopic(name);
        const pending = state?.delegations.filter((d) => d.status === 'PENDING').length || 0;
...
// switch_topic
// ============================================
export async function switchTopic(args) {
    ...
    const pendingDelegations = topic.delegations.filter((d) => d.status === 'PENDING');
    const completeDelegations = topic.delegations.filter((d) => d.status === 'COMPLETE');
...
// delegate_to
// ============================================
export async function delegateTo(args) {
    ...
    const delegation = await createDelegation(currentTopic.name, parsed.agent, parsed.task, parsed.context || {}, parsed.model_tier);
...
// check_delegations
// ============================================
export async function checkDelegations(args) {
    ...
    if (parsed.agent) {
        delegations = delegations.filter((d) => d.agent === parsed.agent);
    }
    ...
    const byStatus = {
        PENDING: delegations.filter((d) => d.status === 'PENDING'),
        IN_PROGRESS: delegations.filter((d) => d.status === 'IN_PROGRESS'),
        COMPLETE: delegations.filter((d) => d.status === 'COMPLETE'),
        BLOCKED: delegations.filter((d) => d.status === 'BLOCKED'),
    };
    let response = `## Delegations for ${topicName}\n\n`;
    if (byStatus.COMPLETE.length > 0) {
        response += `### ✅ Complete (${byStatus.COMPLETE.length})\n`;
        byStatus.COMPLETE.forEach((d) => {
    ...
    if (byStatus.IN_PROGRESS.length > 0) {
        ...
        byStatus.IN_PROGRESS.forEach((d) => {
            response += `- **${d.id}**: ${d.task}\n`;
        });
        response += '\n';
    }

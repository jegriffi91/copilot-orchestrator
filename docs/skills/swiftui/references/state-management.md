...
**Why Private?** Marking state as `private` makes it clear what's created by the view versus what's passed in.
...
**Note**: Mark `@Observable` classes with `@MainActor` to ensure thread safety with SwiftUI, unless your project uses Default Actor Isolation set to `MainActor`.
